labsw_2reg = function(pars, chains,n_dim,n_AR,n_reg,reg_m,reg_s)
{
  ...
  q_range = c(1:n_reg^2)
  mean_range = c(1:((n_m*n_dim)+(n_m*n_AR*n_dim^2)))+max(q_range)
  sigma_range= c(1:(n_s*n_dim^2))+max(mean_range) 
  ...
  index_s=index_lastpar-(n_s*n_dim^2)+1
  for(i in 1:chains)
  {
    chainvals=pars[[i]]
